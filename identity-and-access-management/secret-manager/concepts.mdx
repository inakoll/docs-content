...
An ephemeral policy is a statement that defines the lifetime of a secret and its versions, using either of the following options:
...
The ephemeral policy also allows you to decide whether to disable or delete your secret's versions, once the secret has expired.

<Message type="important">
  - The ephemeral policy can only be applied to a secret at creation, and **cannot be removed** once applied.
  - If applied on a secret, all its versions inherit from the policy.
</Message>
